package response

import (
	"net/http"
	"strings"

	"github.com/gogf/gf/v2/net/ghttp"

	"gbaseadmin/utility/httpmeta"
)

// R 统一响应结构
type R struct {
	Code      int         `json:"code"`
	Message   string      `json:"message"`
	Data      interface{} `json:"data,omitempty"`
	RequestID string      `json:"requestId,omitempty"`
}

// Ok 成功响应
func Ok(r *ghttp.Request, data ...interface{}) {
	resp := buildResponse(r, 0, "ok")
	if len(data) > 0 {
		resp.Data = data[0]
	}
	r.Response.WriteJsonExit(resp)
}

// OkMsg 成功响应（自定义消息）
func OkMsg(r *ghttp.Request, msg string) {
	r.Response.WriteJsonExit(buildResponse(r, 0, resolveMessage("ok", msg)))
}

// Fail 失败响应
func Fail(r *ghttp.Request, msg string) {
	r.Response.WriteJsonExit(buildResponse(r, -1, resolveMessage("error", msg)))
}

// FailCode 失败响应（自定义 code）
func FailCode(r *ghttp.Request, code int, msg string) {
	r.Response.WriteJsonExit(buildResponse(r, code, resolveMessage("error", msg)))
}

// Unauthorized 401 未授权
func Unauthorized(r *ghttp.Request, msg ...string) {
	m := resolveMessage("未登录或登录已过期", msg...)
	r.Response.Status = http.StatusUnauthorized
	r.Response.WriteJsonExit(buildResponse(r, 401, m))
}

// Forbidden 403 无权限
func Forbidden(r *ghttp.Request, msg ...string) {
	m := resolveMessage("无访问权限", msg...)
	r.Response.Status = http.StatusForbidden
	r.Response.WriteJsonExit(buildResponse(r, 403, m))
}

func buildResponse(r *ghttp.Request, code int, message string) R {
	return R{
		Code:      code,
		Message:   message,
		RequestID: httpmeta.RequestID(r),
	}
}

func resolveMessage(fallback string, msg ...string) string {
	for _, item := range msg {
		if value := strings.TrimSpace(item); value != "" {
			return value
		}
	}
	return fallback
}
