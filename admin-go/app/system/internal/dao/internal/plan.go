// ==========================================================================
// Code generated and maintained by GoFrame CLI tool. DO NOT EDIT.
// ==========================================================================

package internal

import (
	"context"

	"github.com/gogf/gf/v2/database/gdb"
	"github.com/gogf/gf/v2/frame/g"
)

// PlanDao is the data access object for the table system_plan.
type PlanDao struct {
	table    string             // table is the underlying table name of the DAO.
	group    string             // group is the database configuration group name of the current DAO.
	columns  PlanColumns  // columns contains all the column names of Table for convenient usage.
	handlers []gdb.ModelHandler // handlers for customized model modification.
}

// PlanColumns defines and stores column names for the table system_plan.
type PlanColumns struct {
	Id          string // 套餐ID（Snowflake）
	Name        string // 套餐名称
	Code        string // 套餐编码
	Description string // 套餐描述
	UserLimit   string // 用户数量上限（-1=无限）
	StorageMb   string // 存储空间上限MB（-1=无限）
	Features    string // 功能开关列表（JSON数组）
	Price       string // 价格（分）
	Sort        string // 排序（升序）
	Status      string // 状态:0=关闭,1=开启
	CreatedBy   string // 创建人ID
	DeptId      string // 所属部门ID
	TenantId    string // 租户
	MerchantId  string // 商户
	CreatedAt   string // 创建时间
	UpdatedAt   string // 更新时间
	DeletedAt   string // 软删除时间
}

// systemPlanColumns holds the columns for the table system_plan.
var systemPlanColumns = PlanColumns{
	Id:          "id",
	Name:        "name",
	Code:        "code",
	Description: "description",
	UserLimit:   "user_limit",
	StorageMb:   "storage_mb",
	Features:    "features",
	Price:       "price",
	Sort:        "sort",
	Status:      "status",
	CreatedBy:   "created_by",
	DeptId:      "dept_id",
	TenantId:    "tenant_id",
	MerchantId:  "merchant_id",
	CreatedAt:   "created_at",
	UpdatedAt:   "updated_at",
	DeletedAt:   "deleted_at",
}

// NewPlanDao creates and returns a new DAO object for table data access.
func NewPlanDao(handlers ...gdb.ModelHandler) *PlanDao {
	return &PlanDao{
		group:    "default",
		table:    "system_plan",
		columns:  systemPlanColumns,
		handlers: handlers,
	}
}

// DB retrieves and returns the underlying raw database management object of the current DAO.
func (dao *PlanDao) DB() gdb.DB {
	return g.DB(dao.group)
}

// Table returns the table name of the current DAO.
func (dao *PlanDao) Table() string {
	return dao.table
}

// Columns returns all column names of the current DAO.
func (dao *PlanDao) Columns() PlanColumns {
	return dao.columns
}

// Group returns the database configuration group name of the current DAO.
func (dao *PlanDao) Group() string {
	return dao.group
}

// Ctx creates and returns a Model for the current DAO. It automatically sets the context for the current operation.
func (dao *PlanDao) Ctx(ctx context.Context) *gdb.Model {
	model := dao.DB().Model(dao.table)
	for _, handler := range dao.handlers {
		model = handler(model)
	}
	return model.Safe().Ctx(ctx)
}

// Transaction wraps the transaction logic using function f.
// It rolls back the transaction and returns the error if function f returns a non-nil error.
// It commits the transaction and returns nil if function f returns nil.
//
// Note: Do not commit or roll back the transaction in function f,
// as it is automatically handled by this function.
func (dao *PlanDao) Transaction(ctx context.Context, f func(ctx context.Context, tx gdb.TX) error) (err error) {
	return dao.Ctx(ctx).Transaction(ctx, f)
}
