...
type MemberUser struct {
	...
	DailyPurchaseLimit uint        `orm:"daily_purchase_limit" description:"本会员每日限购单数（按等级初始化，可单独调整）|search:eq"`            // 本会员每日限购单数（按等级初始化，可单独调整）|search:eq
	TodayPurchaseCount uint        `orm:"today_purchase_count" description:"今日已购单数|search:off"`                            // 今日已购单数|search:off
	LastPurchaseDate   *gtime.Time `orm:"last_purchase_date"   description:"最近购买日期（跨日重置 today_purchase_count）|search:off"` // 最近购买日期（跨日重置 today_purchase_count）|search:off
	TotalPurchaseCount uint        `orm:"total_purchase_count" description:"历史总购单数（用于阶梯返佣判断）|search:off"`                  // 历史总购单数（用于阶梯返佣判断）|search:off
