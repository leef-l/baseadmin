...
type MemberUser struct {
	...
	DailyPurchaseLimit any         // 本会员每日限购单数（按等级初始化，可单独调整）|search:eq
	TodayPurchaseCount any         // 今日已购单数|search:off
	LastPurchaseDate   *gtime.Time // 最近购买日期（跨日重置 today_purchase_count）|search:off
	TotalPurchaseCount any         // 历史总购单数（用于阶梯返佣判断）|search:off
