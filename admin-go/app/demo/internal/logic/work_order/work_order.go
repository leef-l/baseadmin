...
func (s *sWorkOrder) Update(ctx context.Context, in *model.WorkOrderUpdateInput) error {
	...
	if err := middleware.EnsureDataScopedRowAccessible(ctx, dao.DemoWorkOrder.Ctx(ctx), in.ID, dao.DemoWorkOrder.Columns().Id, dao.DemoWorkOrder.Columns().CreatedBy, dao.DemoWorkOrder.Columns().DeptId); err != nil {
		return err
	}
	_, err := dao.DemoWorkOrder.Ctx(ctx).Where(dao.DemoWorkOrder.Columns().Id, in.ID).Where(dao.DemoWorkOrder.Columns().DeletedAt, nil).Data(data).Update()
	return err
}
...
func (s *sWorkOrder) Delete(ctx context.Context, id snowflake.JsonInt64) error {
	...
	if err := middleware.EnsureDataScopedRowAccessible(ctx, dao.DemoWorkOrder.Ctx(ctx), id, dao.DemoWorkOrder.Columns().Id, dao.DemoWorkOrder.Columns().CreatedBy, dao.DemoWorkOrder.Columns().DeptId); err != nil {
		return err
	}
...
func (s *sWorkOrder) BatchDelete(ctx context.Context, ids []snowflake.JsonInt64) error {
	...
	if err := middleware.EnsureDataScopedRowsAccessible(ctx, dao.DemoWorkOrder.Ctx(ctx), normalizedIDs, dao.DemoWorkOrder.Columns().Id, dao.DemoWorkOrder.Columns().CreatedBy, dao.DemoWorkOrder.Columns().DeptId); err != nil {
		return err
	}
...
func (s *sWorkOrder) Detail(ctx context.Context, id snowflake.JsonInt64) (out *model.WorkOrderDetailOutput, err error) {
	...
	if out == nil || out.ID == 0 {
		return nil, gerror.New("体验工单不存在或已删除")
	}
	// 查询客户关联显示
	if out.CustomerID != 0 {
...
func (s *sWorkOrder) Import(ctx context.Context, file *ghttp.UploadFile) (success int, fail int, err error) {
	...
	for {
		...
		idx := 0
		if idx < len(record) {
			data.TicketNo = strings.TrimSpace(record[idx])
		}
		idx++
		if idx < len(record) {
		...
		idx++
		if idx < len(record) {
			data.AttachmentFile = strings.TrimSpace(record[idx])
		}
		idx++
		if idx < len(record) {
			if v, parseErr := strconv.Atoi(strings.TrimSpace(record[idx])); parseErr == nil {
