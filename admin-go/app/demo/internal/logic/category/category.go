...
func normalizeCategoryIDs(ids []snowflake.JsonInt64) []snowflake.JsonInt64 {
	if len(ids) == 0 {
		return nil
	}
...
func (s *sCategory) Create(ctx context.Context, in *model.CategoryCreateInput) error {
	...
	_, err := dao.DemoCategory.Ctx(ctx).Data(do.DemoCategory{
		Id:        id,
...
// BatchDelete 批量软删除体验分类
func (s *sCategory) BatchDelete(ctx context.Context, ids []snowflake.JsonInt64) error {
...
func (s *sCategory) collectDeleteIDs(ctx context.Context, ids []snowflake.JsonInt64) ([]snowflake.JsonInt64, error) {
	normalized := normalizeCategoryIDs(ids)
	if len(normalized) == 0 {
		return nil, nil
	}
...
func (s *sCategory) collectChildIDs(ctx context.Context, parentID snowflake.JsonInt64) ([]snowflake.JsonInt64, error) {
...
func (s *sCategory) applyListFilter(ctx context.Context, in *model.CategoryListInput) *gdb.Model {
	m := dao.DemoCategory.Ctx(ctx).Where(dao.DemoCategory.Columns().DeletedAt, nil)
	m = middleware.ApplyTenantScopeToModel(ctx, m, dao.DemoCategory.Columns().TenantId, dao.DemoCategory.Columns().MerchantId)
	if in.Name != "" {
		m = m.WhereLike(dao.DemoCategory.Columns().Name, "%"+in.Name+"%")
	}
	if in.ParentID != nil {
		m = m.Where(dao.DemoCategory.Columns().ParentId, *in.ParentID)
	}
	if in.TenantID != nil {
		m = m.Where(dao.DemoCategory.Columns().TenantId, *in.TenantID)
	}
	if in.MerchantID != nil {
		m = m.Where(dao.DemoCategory.Columns().MerchantId, *in.MerchantID)
	}
	if in.Status != nil {
		m = m.Where(dao.DemoCategory.Columns().Status, *in.Status)
	}
	if in.StartTime != "" {
		m = m.WhereGTE(dao.DemoCategory.Columns().CreatedAt, in.StartTime)
	}
	if in.EndTime != "" {
		m = m.WhereLTE(dao.DemoCategory.Columns().CreatedAt, in.EndTime)
	}
	// 数据权限过滤
...
func (s *sCategory) fillRefFields(ctx context.Context, list []*model.CategoryListOutput) {
	{
...
func (s *sCategory) isAllowedOrderField(field string) bool {
	allowed := map[string]bool{
		dao.DemoCategory.Columns().Id:        true,
		dao.DemoCategory.Columns().CreatedAt: true,
		dao.DemoCategory.Columns().Sort:      true,
		dao.DemoCategory.Columns().Status:    true,
		dao.DemoCategory.Columns().Name: true,
	}
	return allowed[field]
}

func (s *sCategory) applyListOrder(m *gdb.Model, orderBy, orderDir string) *gdb.Model {
	...
	return m.OrderAsc(dao.DemoCategory.Columns().Sort).OrderDesc(dao.DemoCategory.Columns().Id)
}

// Export 导出体验分类（不分页）
func (s *sCategory) Export(ctx context.Context, in *model.CategoryListInput) (list []*model.CategoryListOutput, err error) {
	if in == nil {
		in = &model.CategoryListInput{}
	}
	m := s.applyListFilter(ctx, in)
...
// Tree 获取体验分类树形结构
func (s *sCategory) Tree(ctx context.Context, in *model.CategoryTreeInput) (tree []*model.CategoryTreeOutput, err error) {
	var list []*model.CategoryTreeOutput
	if in == nil {
		in = &model.CategoryTreeInput{}
	}
	m := dao.DemoCategory.Ctx(ctx).Where(dao.DemoCategory.Columns().DeletedAt, nil)
	m = middleware.ApplyTenantScopeToModel(ctx, m, dao.DemoCategory.Columns().TenantId, dao.DemoCategory.Columns().MerchantId)
	if in.Name != "" {
	...
	{
		idSet := make(map[int64]struct{})
		var collectIDs func(items []*model.CategoryTreeOutput)
		collectIDs = func(items []*model.CategoryTreeOutput) {
		...
		if len(idSet) > 0 {
			...
			if queryErr == nil {
				...
				var fillRef func(items []*model.CategoryTreeOutput)
				fillRef = func(items []*model.CategoryTreeOutput) {
					for _, item := range items {
						if val, ok := refMap[int64(item.TenantID)]; ok {
							item.TenantName = val
						}
