...
{{- else if eq .Component "FileUpload"}}
      <DescriptionsItem {{if $isScopeField}}v-if="isPlatformSuperAdmin" {{end}}label="{{.ShortLabel}}">
        <a v-if="detail.{{.NameLower}} && /^https?:\/\//i.test(detail.{{.NameLower}})" :href="detail.{{.NameLower}}" target="_blank" rel="noreferrer noopener">查看文件</a>
        <span v-else-if="detail.{{.NameLower}}">{{"{{"}} detail.{{.NameLower}} {{"}}"}}</span>
        <span v-else>-</span>
      </DescriptionsItem>
{{- else if eq .Component "InputUrl"}}
      <DescriptionsItem {{if $isScopeField}}v-if="isPlatformSuperAdmin" {{end}}label="{{.ShortLabel}}">
        <a v-if="detail.{{.NameLower}} && /^https?:\/\//i.test(detail.{{.NameLower}})" :href="detail.{{.NameLower}}" target="_blank" rel="noreferrer noopener">{{"{{"}} detail.{{.NameLower}} {{"}}"}}</a>
        <span v-else-if="detail.{{.NameLower}}">{{"{{"}} detail.{{.NameLower}} {{"}}"}}</span>
        <span v-else>-</span>
      </DescriptionsItem>
