...
- **强制权限归属字段** — codegen 解析表结构时要求 CRUD 表同时具备 `tenant_id`、`merchant_id`、`created_by` 和 `dept_id`，缺任意一个直接失败，避免生成没有租户、商户和部门数据权限闭环的接口
- **同步文档契约** — README、字段备注规则、AI 执行手册、迁移说明和 SaaS 文档统一改为“四个字段必需”，不再使用“有一个字段就触发”的描述

## v1.6.8 — 2026-04-28

### 文档

- **补充字段备注规范** — 新增 `docs/字段备注与生成规则.md`，系统说明标签、Tooltip、枚举、字典、搜索、外键、校验、隐藏字段和 SaaS 数据权限的生成规则
- **补齐 AI 执行手册** — 新增 `docs/Codegen-AI执行手册.md`，明确 codegen 排查路径、受限命令入口、真源边界和模板修改检查点
- **README 更新验证约束** — 当前服务器文档不再引导执行 `go test`，改为受限 `go run verify_codegen.go` 和 `dry-run --manifest-out` 作为基础验证入口
