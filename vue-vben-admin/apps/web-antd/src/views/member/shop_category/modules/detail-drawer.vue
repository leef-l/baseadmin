...
import { getShopCategoryDetail } from '#/api/member/shop_category';
...
const detail = ref<ShopCategoryItem | null>(null);
...
const [Modal, modalApi] = useVbenModal({
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    if (data?.id) {
      modalApi.setState({ title: '商城商品分类详情' });
      try {
        const res = await getShopCategoryDetail(data.id);
...
<template>
  <Modal class="w-[600px]">
    <Descriptions v-if="detail" bordered :column="1" size="small">
      ...
      <DescriptionsItem label="上级分类">{{ detail.shopCategoryName || '-' }}</DescriptionsItem>
      <DescriptionsItem label="分类名称">{{ displayValue(detail.name) }}</DescriptionsItem>
      <DescriptionsItem label="分类图标">{{ displayValue(detail.icon) }}</DescriptionsItem>
