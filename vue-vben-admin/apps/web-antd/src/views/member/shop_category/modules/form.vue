...
import {
  ...
  updateShopCategory,
  getShopCategoryTree,
} from '#/api/member/shop_category';
import type {
  ShopCategoryCreateParams,
  ShopCategoryUpdateParams,
  ShopCategoryItem
} from '#/api/member/shop_category/types';
const treeData = ref<ShopCategoryItem[]>([]);
...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    ...
    try {
      if (isEdit.value) {
        await updateShopCategory({ id: editId.value, ...values } as ShopCategoryUpdateParams);
        message.success('更新成功');
      } else {
        await createShopCategory(values);
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    // 加载树形数据
    try {
      const res = await getShopCategoryTree();
    ...
    if (data?.id) {
      ...
      modalApi.setState({ title: '编辑商城商品分类' });
      try {
        const detail = await getShopCategoryDetail(data.id);
        if (currentOpenToken !== openToken.value) {
          return;
        }
        if (detail) {
      ...
      isEdit.value = false;
      editId.value = '';
      modalApi.setState({ title: '新建商城商品分类' });
    }
  },
});
</script>
