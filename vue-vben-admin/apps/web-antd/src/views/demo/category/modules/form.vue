...
const [Form, formApi] = useVbenForm({
  ...
  schema: [
    ...
    },
    {
      ...
      label: '分类名称',
      rules: 'required',
      componentProps: { placeholder: '请输入分类名称', maxlength: 80 },
    },
    {
...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    ...
    try {
      if (isEdit.value) {
        await updateCategory({ id: editId.value, ...values } as CategoryUpdateParams);
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    if (data?.id) {
      ...
      modalApi.setState({ title: '编辑体验分类' });
      try {
      ...
      isEdit.value = false;
      editId.value = '';
      modalApi.setState({ title: '新建体验分类' });
    }
  },
});
</script>
