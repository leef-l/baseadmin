...
import { getCategoryTree, deleteCategory, batchDeleteCategory, exportCategory, batchUpdateCategory } from '#/api/demo/category';
...
const canBatchDelete = hasAccessByCodes(['demo:category:batch-delete']);
...
/** 查看 */
function handleView(row: CategoryItem) {
...
function handleBatchDelete() {
  const ids = getGridSelectedIds<CategoryItem>(gridApi.grid as any);
  if (ids.length === 0) {
  ...
  Modal.confirm({
    title: '确认批量删除',
    content: `确定要删除选中的 ${ids.length} 条体验分类吗？`,
    okType: 'danger',
    async onOk() {
...
function handleBatchUpdateStatus() {
  ...
  const rows = gridApi.grid.getCheckboxRecords() as CategoryItem[];
  Modal.confirm({
    title: '批量修改状态',
    content: `确定要将选中的 ${ids.length} 条数据的状态切换吗？`,
    async onOk() {
      const newStatus = rows[0]?.status === 1 ? 0 : 1;
      await batchUpdateCategory({ ids, status: newStatus });
