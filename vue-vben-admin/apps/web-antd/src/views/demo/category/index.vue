...
function handleBatchDelete() {
  ...
  Modal.confirm({
    title: '确认批量删除',
    content: `确定要删除选中的 ${rows.length} 条体验分类吗？`,
    okType: 'danger',
    async onOk() {
      await batchDeleteCategory(rows.map((r: CategoryItem) => r.id));
...
function handleBatchUpdateStatus() {
  ...
  Modal.confirm({
    ...
    async onOk() {
      ...
      await batchUpdateCategory({ ids: rows.map((r: CategoryItem) => r.id), status: newStatus });
