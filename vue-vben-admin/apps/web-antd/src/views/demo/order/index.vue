...
function handleBatchDelete() {
  ...
  Modal.confirm({
    title: '确认批量删除',
    content: `确定要删除选中的 ${rows.length} 条体验订单吗？`,
    okType: 'danger',
    async onOk() {
      await batchDeleteOrder(rows.map((r: OrderItem) => r.id));
...
function handleBatchUpdateStatus() {
  ...
  Modal.confirm({
    ...
    async onOk() {
      ...
      await batchUpdateOrder({ ids: rows.map((r: OrderItem) => r.id), status: newStatus });
