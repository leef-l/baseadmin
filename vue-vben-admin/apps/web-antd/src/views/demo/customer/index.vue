...
function handleBatchDelete() {
  ...
  Modal.confirm({
    title: '确认批量删除',
    content: `确定要删除选中的 ${rows.length} 条体验客户吗？`,
    okType: 'danger',
    async onOk() {
      await batchDeleteCustomer(rows.map((r: CustomerItem) => r.id));
...
function handleBatchUpdateStatus() {
  ...
  Modal.confirm({
    ...
    async onOk() {
      ...
      await batchUpdateCustomer({ ids: rows.map((r: CustomerItem) => r.id), status: newStatus });
