...
import { getCustomerList, deleteCustomer, batchDeleteCustomer, exportCustomer, importCustomer, downloadImportTemplateCustomer, batchUpdateCustomer } from '#/api/demo/customer';
...
const canBatchDelete = hasAccessByCodes(['demo:customer:batch-delete']);
...
const formOptions: VbenFormProps = {
  ...
  schema: [
    ...
    {
      component: 'Input',
      componentProps: { placeholder: '请输入客户编号', allowClear: true },
...
const gridOptions: VxeGridProps<CustomerItem> = {
  ...
  columns: [
    ...
    { field: 'registeredAt', title: '注册时间', width: 180, formatter: 'formatDateTime' },
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page, sorts }, formValues) => {
        ...
        if (params.registeredAtRange && params.registeredAtRange.length === 2) {
          params.registeredAtStart = params.registeredAtRange[0];
          params.registeredAtEnd = params.registeredAtRange[1];
        }
        delete params.registeredAtRange;
...
/** 查看 */
function handleView(row: CustomerItem) {
...
function handleBatchDelete() {
  const ids = getGridSelectedIds<CustomerItem>(gridApi.grid as any);
  if (ids.length === 0) {
  ...
  Modal.confirm({
    title: '确认批量删除',
    content: `确定要删除选中的 ${ids.length} 条体验客户吗？`,
    okType: 'danger',
    async onOk() {
...
async function handleExport() {
  try {
    ...
    const blob = await exportCustomer(params);
    downloadFileFromBlob({ fileName: '体验客户.csv', source: blob as Blob });
...
async function handleDownloadTemplate() {
  try {
    const blob = await downloadImportTemplateCustomer();
    downloadFileFromBlob({ fileName: '体验客户导入模板.csv', source: blob as Blob });
  } catch {
...
function handleBatchUpdateStatus() {
  ...
  const rows = gridApi.grid.getCheckboxRecords() as CustomerItem[];
  Modal.confirm({
    title: '批量修改状态',
    content: `确定要将选中的 ${ids.length} 条数据的状态切换吗？`,
    async onOk() {
      const newStatus = rows[0]?.status === 1 ? 0 : 1;
      await batchUpdateCustomer({ ids, status: newStatus });
    ...
    <Grid>
      ...
      <template #gender_cell="{ row }">
        <Tag :color="getGenderColor(row.gender)">
          {{ getEnumLabel(genderMap, row.gender) }}
        </Tag>
      </template>
      <template #level_cell="{ row }">
        <Tag :color="getLevelColor(row.level)">
          {{ getEnumLabel(levelMap, row.level) }}
        </Tag>
      </template>
      ...
      <template #isVip_cell="{ row }">
        <Tag :color="getIsVipColor(row.isVip)">
          {{ getEnumLabel(isVipMap, row.isVip) }}
        </Tag>
      </template>
