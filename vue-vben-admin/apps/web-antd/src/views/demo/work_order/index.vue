...
function handleBatchDelete() {
  const ids = getGridSelectedIds<WorkOrderItem>(gridApi.grid as any);
  if (ids.length === 0) {
  ...
  Modal.confirm({
    title: '确认批量删除',
    content: `确定要删除选中的 ${ids.length} 条体验工单吗？`,
    okType: 'danger',
    async onOk() {
...
function handleBatchUpdateStatus() {
  ...
  const rows = gridApi.grid.getCheckboxRecords() as WorkOrderItem[];
  Modal.confirm({
    title: '批量修改状态',
    content: `确定要将选中的 ${ids.length} 条数据的状态切换吗？`,
    async onOk() {
      const newStatus = rows[0]?.status === 1 ? 0 : 1;
      await batchUpdateWorkOrder({ ids, status: newStatus });
  ...
  <Page auto-content-height>
    ...
    <Grid>
      <template #toolbar-actions>
        <Button v-access:code="'demo:work_order:create'" type="primary" @click="handleCreate">新建</Button>
      ...
      <template #action="{ row }">
        <Button v-access:code="'demo:work_order:detail'" type="link" size="small" @click="handleView(row)">查看</Button>
        <Button v-access:code="'demo:work_order:update'" type="link" size="small" @click="handleEdit(row)">编辑</Button>
        <Button v-access:code="'demo:work_order:delete'" type="link" danger size="small" @click="handleDelete(row)">删除</Button>
      </template>
    </Grid>
  </Page>
</template>
