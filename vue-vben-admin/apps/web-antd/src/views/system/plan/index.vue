...
import { getPlanList, deletePlan, batchDeletePlan, exportPlan, importPlan, downloadImportTemplatePlan, batchUpdatePlan } from '#/api/system/plan';
import { getTenantList } from '#/api/system/tenant';
import { getMerchantList } from '#/api/system/merchant';
import type { PlanItem } from '#/api/system/plan/types';
...
const canBatchDelete = hasAccessByCodes(['system:plan:batch-delete']);
const canDelete = hasAccessByCodes(['system:plan:delete']);
const canDetail = hasAccessByCodes(['system:plan:detail']);
const canUpdate = hasAccessByCodes(['system:plan:update']);
...
const formOptions: VbenFormProps = {
  ...
  schema: [
    ...
    {
      component: 'Input',
      componentProps: { placeholder: '请输入套餐编码', allowClear: true },
      fieldName: 'code',
      label: '套餐编码',
    },
    {
    ...
    {
      component: 'Input',
      componentProps: { placeholder: '请输入套餐描述', allowClear: true },
...
const gridOptions: VxeGridProps<PlanItem> = {
  ...
  columns: [
    ...
    { field: 'name', title: '套餐名称', sortable: true },
    { field: 'code', title: '套餐编码' },
    { field: 'description', title: '套餐描述', sortable: true },
    { field: 'userLimit', title: '用户数量上限', slots: { header: tooltipHeader('用户数量上限', '-1=无限') } },
    { field: 'storageMb', title: '存储空间上限MB', slots: { header: tooltipHeader('存储空间上限MB', '-1=无限') } },
    { field: 'features', title: '功能开关列表', slots: { header: tooltipHeader('功能开关列表', 'JSON数组') } },
    { field: 'price', title: '价格', slots: { header: tooltipHeader('价格', '分') }, width: 120, formatter: ({ cellValue }: any) => cellValue != null ? (cellValue / 100).toFixed(2) : '-', sortable: true },
...
function handleBatchDelete() {
  const ids = getGridSelectedIds<PlanItem>(gridApi.grid as any);
  if (ids.length === 0) {
  ...
  Modal.confirm({
    title: '确认批量删除',
    content: `确定要删除选中的 ${ids.length} 条套餐表吗？`,
    okType: 'danger',
    async onOk() {
...
async function handleExport() {
  try {
    ...
    const blob = await exportPlan(params);
    downloadFileFromBlob({ fileName: '套餐表.csv', source: blob as Blob });
...
async function handleImportChange(event: Event) {
  ...
  try {
    const res = await importPlan(formData);
...
async function handleDownloadTemplate() {
  try {
    const blob = await downloadImportTemplatePlan();
    downloadFileFromBlob({ fileName: '套餐表导入模板.csv', source: blob as Blob });
  } catch {
...
function handleBatchUpdateStatus() {
  ...
  const rows = gridApi.grid.getCheckboxRecords() as PlanItem[];
  Modal.confirm({
    title: '批量修改状态',
    content: `确定要将选中的 ${ids.length} 条数据的状态切换吗？`,
    async onOk() {
      const newStatus = rows[0]?.status === 1 ? 0 : 1;
      await batchUpdatePlan({ ids, status: newStatus });
...
function getRowActions(row: PlanItem): ActionMoreItem[] {
  return [
    {
      key: 'detail',
      label: '查看',
  ...
  <Page auto-content-height>
    ...
    <Grid>
      <template #toolbar-actions>
        <Button v-access:code="'system:plan:create'" class="mr-2" type="primary" @click="handleCreate">新建</Button>
        <Button v-access:code="'system:plan:batch-delete'" danger class="mr-2" @click="handleBatchDelete">批量删除</Button>
        <Button v-access:code="'system:plan:export'" class="mr-2" @click="handleExport">导出</Button>
        <Button v-access:code="'system:plan:import'" class="mr-2" @click="handleImportTrigger">导入</Button>
        <Button v-access:code="'system:plan:import'" class="mr-2" @click="handleDownloadTemplate">模板下载</Button>
        <Button v-access:code="'system:plan:batch-update'" class="mr-2" @click="handleBatchUpdateStatus">批量修改状态</Button>
      </template>
