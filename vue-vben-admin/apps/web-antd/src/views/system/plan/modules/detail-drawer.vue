...
import { getPlanDetail } from '#/api/system/plan';
...
<template>
  <Modal class="w-[600px]">
    <Descriptions v-if="detail" bordered :column="1" size="small">
      ...
      <DescriptionsItem label="套餐名称">{{ displayValue(detail.name) }}</DescriptionsItem>
      <DescriptionsItem label="套餐编码">{{ displayValue(detail.code) }}</DescriptionsItem>
      <DescriptionsItem label="套餐描述">{{ displayValue(detail.description) }}</DescriptionsItem>
      <DescriptionsItem label="用户数量上限">{{ displayValue(detail.userLimit) }}</DescriptionsItem>
      <DescriptionsItem label="存储空间上限MB">{{ displayValue(detail.storageMb) }}</DescriptionsItem>
      <DescriptionsItem label="功能开关列表">{{ displayValue(detail.features) }}</DescriptionsItem>
      <DescriptionsItem label="价格">{{ detail.price != null ? (detail.price / 100).toFixed(2) : '-' }}</DescriptionsItem>
