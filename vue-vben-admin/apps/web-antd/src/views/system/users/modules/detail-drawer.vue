...
import { getUsersDetail } from '#/api/system/users';
...
const detail = ref<UsersItem | null>(null);
...
const [Modal, modalApi] = useVbenModal({
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    if (data?.id) {
      modalApi.setState({ title: '用户表详情' });
      try {
        const res = await getUsersDetail(data.id);
...
<template>
  <Modal class="w-[600px]">
    <Descriptions v-if="detail" bordered :column="1" size="small">
      ...
      <DescriptionsItem label="登录用户名">{{ displayValue(detail.username) }}</DescriptionsItem>
      <DescriptionsItem label="昵称/显示名">{{ displayValue(detail.nickname) }}</DescriptionsItem>
      <DescriptionsItem label="邮箱地址">{{ displayValue(detail.email) }}</DescriptionsItem>
      <DescriptionsItem label="头像图片 URL">
