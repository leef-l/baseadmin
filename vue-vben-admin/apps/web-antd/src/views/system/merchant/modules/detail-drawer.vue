...
const [Modal, modalApi] = useVbenModal({
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    if (data?.id) {
      modalApi.setState({ title: '商户表详情' });
      try {
        const res = await getMerchantDetail(data.id);
...
<template>
  <Modal class="w-[600px]">
    <Descriptions v-if="detail" bordered :column="1" size="small">
      ...
      <DescriptionsItem label="商户名称">{{ displayValue(detail.name) }}</DescriptionsItem>
      <DescriptionsItem label="商户编码">{{ displayValue(detail.code) }}</DescriptionsItem>
      ...
      <DescriptionsItem label="商户地址">{{ displayValue(detail.address) }}</DescriptionsItem>
