...
import { getTenantDetail } from '#/api/system/tenant';
...
const [Modal, modalApi] = useVbenModal({
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    if (data?.id) {
      modalApi.setState({ title: '租户表详情' });
      try {
        const res = await getTenantDetail(data.id);
...
<template>
  <Modal class="w-[600px]">
    <Descriptions v-if="detail" bordered :column="1" size="small">
      ...
      <DescriptionsItem label="租户名称">{{ displayValue(detail.name) }}</DescriptionsItem>
      <DescriptionsItem label="租户编码">{{ displayValue(detail.code) }}</DescriptionsItem>
      <DescriptionsItem label="联系人">{{ displayValue(detail.contactName) }}</DescriptionsItem>
      <DescriptionsItem label="联系电话">{{ displayValue(detail.contactPhone) }}</DescriptionsItem>
      <DescriptionsItem label="租户域名">{{ displayValue(detail.domain) }}</DescriptionsItem>
      ...
      <DescriptionsItem label="到期时间">{{ displayValue(detail.expireAt) }}</DescriptionsItem>
