...
import { getDeptDetail } from '#/api/system/dept';
...
const detail = ref<DeptItem | null>(null);
...
<template>
  <Modal class="w-[600px]">
    <Descriptions v-if="detail" bordered :column="1" size="small">
      ...
      <DescriptionsItem label="上级部门ID，0 表示顶级部门">{{ detail.deptTitle || '-' }}</DescriptionsItem>
      <DescriptionsItem label="部门名称">{{ displayValue(detail.title) }}</DescriptionsItem>
      <DescriptionsItem label="部门负责人姓名">{{ displayValue(detail.username) }}</DescriptionsItem>
      <DescriptionsItem label="负责人邮箱">{{ displayValue(detail.email) }}</DescriptionsItem>
