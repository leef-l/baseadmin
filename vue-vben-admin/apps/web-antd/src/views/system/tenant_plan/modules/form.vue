...
import {
  ...
  createTenantPlan,
  updateTenantPlan,
} from '#/api/system/tenant_plan';
import type {
  TenantPlanCreateParams,
  TenantPlanUpdateParams
} from '#/api/system/tenant_plan/types';
...
const planIDOptions = ref<{ label: string; value: string | number }[]>([]);
...
const [Form, formApi] = useVbenForm({
  ...
  schema: [
    ...
    },
    {
      ...
      label: '套餐ID',
      rules: 'selectRequired',
      componentProps: { options: [], placeholder: '请选择套餐ID', allowClear: true, class: 'w-full' },
    },
    {
      component: 'DatePicker',
      fieldName: 'startAt',
      label: '生效时间',
      componentProps: { showTime: true, placeholder: '请选择生效时间', class: 'w-full', valueFormat: 'YYYY-MM-DD HH:mm:ss' },
    },
    {
      component: 'DatePicker',
      fieldName: 'expireAt',
...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    ...
    try {
      if (isEdit.value) {
        await updateTenantPlan({ id: editId.value, ...values } as TenantPlanUpdateParams);
        message.success('更新成功');
      } else {
        await createTenantPlan(values);
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    try {
      ...
      formApi.updateSchema([
        {
          fieldName: 'planID',
          componentProps: { options: planIDOptions.value },
        },
      ]);
    } catch {
      // ignore
    ...
    if (data?.id) {
      ...
      modalApi.setState({ title: '编辑租户套餐订阅表' });
      try {
        const detail = await getTenantPlanDetail(data.id);
        if (currentOpenToken !== openToken.value) {
          return;
        }
        if (detail) {
      ...
      isEdit.value = false;
      editId.value = '';
      modalApi.setState({ title: '新建租户套餐订阅表' });
    }
  },
});
</script>
