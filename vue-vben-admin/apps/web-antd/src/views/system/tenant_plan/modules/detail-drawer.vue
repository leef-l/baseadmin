...
import { getTenantPlanDetail } from '#/api/system/tenant_plan';
...
const detail = ref<TenantPlanItem | null>(null);
...
const [Modal, modalApi] = useVbenModal({
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    if (data?.id) {
      modalApi.setState({ title: '租户套餐订阅表详情' });
      try {
        const res = await getTenantPlanDetail(data.id);
...
<template>
  <Modal class="w-[600px]">
    <Descriptions v-if="detail" bordered :column="1" size="small">
      ...
      <DescriptionsItem label="生效时间">{{ displayValue(detail.startAt) }}</DescriptionsItem>
