...
import { getTenantPlanList, deleteTenantPlan, batchDeleteTenantPlan, exportTenantPlan, importTenantPlan, downloadImportTemplateTenantPlan, batchUpdateTenantPlan } from '#/api/system/tenant_plan';
import { getTenantList } from '#/api/system/tenant';
import { getPlanList } from '#/api/system/plan';
import { getMerchantList } from '#/api/system/merchant';
import type { TenantPlanItem } from '#/api/system/tenant_plan/types';
...
const canBatchDelete = hasAccessByCodes(['system:tenant_plan:batch-delete']);
const canDelete = hasAccessByCodes(['system:tenant_plan:delete']);
const canDetail = hasAccessByCodes(['system:tenant_plan:detail']);
const canUpdate = hasAccessByCodes(['system:tenant_plan:update']);
...
/** 表格列配置 */
const gridOptions: VxeGridProps<TenantPlanItem> = {
  ...
  columns: [
    ...
    { field: 'planName', title: '套餐ID' },
    { field: 'status', title: '状态', width: 120, slots: { default: 'status_cell' }, sortable: true },
    { field: 'remark', title: '备注', sortable: true },
    ...(isPlatformSuperAdmin.value ? [
    { field: 'merchantName', title: '商户' },
    ] : []),
    { field: 'startAt', title: '生效时间', width: 180, formatter: 'formatDateTime' },
    { field: 'expireAt', title: '到期时间', width: 180, formatter: 'formatDateTime' },
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page, sorts }, formValues) => {
        ...
        const res = await getTenantPlanList(params as any);
...
async function initSearchOptions() {
  ...
  try {
    const planIDRes = await getPlanList({ pageNum: 1, pageSize: 500 });
    gridApi.formApi.updateSchema([
      {
        fieldName: 'planID',
        componentProps: {
          options: (planIDRes?.list ?? []).map((item: any) => ({
            label: item.name || item.id,
            value: item.id,
          })),
        },
...
/** 查看 */
function handleView(row: TenantPlanItem) {
  detailDrawerApi.setData({ id: row.id }).open();
}

/** 编辑 */
function handleEdit(row: TenantPlanItem) {
  formModalApi.setData({ id: row.id }).open();
}

/** 删除 */
function handleDelete(row: TenantPlanItem) {
  Modal.confirm({
    title: '确认删除',
    content: '确定要删除该租户套餐订阅表吗？',
    okType: 'danger',
    async onOk() {
      await deleteTenantPlan(row.id);
...
function handleBatchDelete() {
  const ids = getGridSelectedIds<TenantPlanItem>(gridApi.grid as any);
  if (ids.length === 0) {
  ...
  Modal.confirm({
    title: '确认批量删除',
    content: `确定要删除选中的 ${ids.length} 条租户套餐订阅表吗？`,
    okType: 'danger',
    async onOk() {
      await batchDeleteTenantPlan(ids);
...
async function handleExport() {
  try {
    ...
    const blob = await exportTenantPlan(params);
    downloadFileFromBlob({ fileName: '租户套餐订阅表.csv', source: blob as Blob });
...
async function handleImportChange(event: Event) {
  ...
  try {
    const res = await importTenantPlan(formData);
...
async function handleDownloadTemplate() {
  try {
    const blob = await downloadImportTemplateTenantPlan();
    downloadFileFromBlob({ fileName: '租户套餐订阅表导入模板.csv', source: blob as Blob });
  } catch {
...
function handleBatchUpdateStatus() {
  ...
  const rows = gridApi.grid.getCheckboxRecords() as TenantPlanItem[];
  Modal.confirm({
    title: '批量修改状态',
    content: `确定要将选中的 ${ids.length} 条数据的状态切换吗？`,
    async onOk() {
      const newStatus = rows[0]?.status === 1 ? 0 : 1;
      await batchUpdateTenantPlan({ ids, status: newStatus });
...
function getRowActions(row: TenantPlanItem): ActionMoreItem[] {
  return [
    {
      key: 'detail',
      label: '查看',
  ...
  <Page auto-content-height>
    ...
    <Grid>
      <template #toolbar-actions>
        <Button v-access:code="'system:tenant_plan:create'" class="mr-2" type="primary" @click="handleCreate">新建</Button>
        <Button v-access:code="'system:tenant_plan:batch-delete'" danger class="mr-2" @click="handleBatchDelete">批量删除</Button>
        <Button v-access:code="'system:tenant_plan:export'" class="mr-2" @click="handleExport">导出</Button>
        <Button v-access:code="'system:tenant_plan:import'" class="mr-2" @click="handleImportTrigger">导入</Button>
        <Button v-access:code="'system:tenant_plan:import'" class="mr-2" @click="handleDownloadTemplate">模板下载</Button>
        <Button v-access:code="'system:tenant_plan:batch-update'" class="mr-2" @click="handleBatchUpdateStatus">批量修改状态</Button>
      </template>
