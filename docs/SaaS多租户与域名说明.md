...
3. 匹配后把域名归属写入请求上下文
4. `Auth` 中间件通过 `DomainScopeAllows` 校验当前 JWT 的租户/商户归属是否允许访问该域名

因此，一个租户域名不能被其他租户账号拿着 JWT 直接访问。

### 域名严格模式

配置 `domain.strictMode: true` 可启用严格模式。在严格模式下，如果 `system_domain` 中已存在任何已验证的域名记录（缓存 1 分钟），那么通过未绑定域名的请求将被拒绝。这适用于需要强制所有访问都走已绑定域名的场景。

默认不开启严格模式，未匹配域名的请求正常放行。
